        ...
        for idx,article in enumerate(iterator):
            ...
            if len(tokenized_text) > 511:
                ...
                for i in range(split_index+1):
                    ...
                    tokens_tensor = tokens_tensor.cuda(cuda_num) 
...
def inference(dataset_name,checkpoint,loop):
    ...
    for model_class, tokenizer_class, pretrained_weights in MODELS:
        ...
        np.savetxt("./datasets/%s_data/%s_tuned_neg_%s.csv"%(pretrained_weights,dataset_name,loop), representations[:class_one_len], delimiter=",")
        np.savetxt("./datasets/%s_data/%s_tuned_pos_%s.csv"%(pretrained_weights,dataset_name,loop), representations[class_one_len:], delimiter=",")

        np.savetxt("./datasets/%s_data/%s_tuned_neg_cls_%s.csv"%(pretrained_weights,dataset_name,loop), representations_cls[:class_one_len], delimiter=",")
        np.savetxt("./datasets/%s_data/%s_tuned_pos_cls_%s.csv"%(pretrained_weights,dataset_name,loop), representations_cls[class_one_len:], delimiter=",")
