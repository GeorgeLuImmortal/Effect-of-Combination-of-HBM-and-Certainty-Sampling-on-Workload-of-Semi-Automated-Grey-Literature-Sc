...
def active_process_svm(arg):
    ...
    if arg.initial: 
        ...
        bin_count, bin_count_ori = np.bincount(Y_train), np.bincount(Y_train)
    ...
    for seed in random_states:
        ...
        for loop in range(options.loop):
            ...
            if loop==0:
                ...
                df.to_csv(f"./outputs/{dataset}/raw_{text_rep}_{seed}_fastread_result.csv",index=False)
                df_p[seed] = [permutation for permutation in permutations]
                df_p.to_csv(f"./outputs/{dataset}/permutation_{text_rep}_{seed}_fastread_result.csv",index=False)
                df_m[seed] = [metric for metric in raw_metrics]
                df_m.to_csv(f"./outputs/{dataset}/metrics_{text_rep}_{seed}_fastread_result.csv",index=False)
                ...
                if num_pos_train>30:
                    ...
                    permutation = sample_candidates(num_candidate=options.num_candidate,permutation=au_permutation,selection_method=selection_method,y_prob=y_prob,loop=loop)
                    permutations.append(permutation)
